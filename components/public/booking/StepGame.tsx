  ...
  return (
    <div className="space-y-6">
      ...
      {data.gameId && data.gameTitle && (
        <div className="flex items-center justify-between p-4 bg-teal-50 rounded-token border border-teal-200">
          ...
          <button
            ...
            className="p-2 text-teal-600 hover:text-teal-800 hover:bg-teal-100 rounded-full transition-colors"
      ...
      <div className="relative">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-[color:var(--color-ink-secondary)] pointer-events-none" />
        <input
          type="text"
          value={searchQuery}
          onChange={e => setSearchQuery(e.target.value)}
          placeholder="Search games..."
          className="w-full rounded-token border border-[color:var(--color-structure)] bg-[color:var(--color-elevated)] pl-11 pr-4 py-3 text-base shadow-card focus-ring"
        />
        {searchQuery && (
          <button
            type="button"
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {filteredGames.map(game => (
                  <button
                    ...
                    className={cn(
                      'w-full flex items-center gap-3 p-3 rounded-token border transition-colors touch-manipulation text-left',
                      data.gameId === game.id
      ...
      <div className="flex gap-3 pt-2">
        ...
        {data.gameId ? (
          ...
          <button
            type="button"
            onClick={handleSkip}
            className="flex-[2] py-3 rounded-token font-medium text-base flex items-center justify-center gap-2 transition-colors touch-manipulation border border-[color:var(--color-structure)] text-[color:var(--color-ink-secondary)] hover:bg-[color:var(--color-muted)]"
          >
            Skip for now
            <ChevronRight className="w-5 h-5" />
          </button>
